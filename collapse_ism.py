...
def collapse_ISM(class_df, junc_df, expr_df=None):
    ...
    for at, group in class_df.groupby("associated_transcript"):
        ...
        if "full_splice_match" in sc:
            ...
            ism = group[group["structural_category"] == "incomplete_splice_match"]
            ...
            if len(fsm) == 1:
                ...
                fsm_sorted = fsm.sort_values(by="subcategory", key=lambda col: col.map(lambda x: priority.index(x) if x in priority else len(priority)))
        ...
        # Case 2: Only ISMs
            ...
            ism_sorted = group.sort_values(by="subcategory", key=lambda col: col.map(lambda x: priority.index(x) if x in priority else len(priority)))
...
def main():
    ...
    # Save new files
    def save_file(df, in_path):
        name, ext = os.path.splitext(os.path.basename(in_path))
        out_path = os.path.join(args.out, f"{name}.ISMcollapsed{ext}")
